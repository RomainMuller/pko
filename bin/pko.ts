...
async function main(argv: string[]): Promise<number> {
  // Disable colors if $PKO_NO_COLOR is set to a truthy value
  if (process.env.PKO_NO_COLOR) {
    colors.disable();
  }
  // Make verbose if $PKO_VERBOSE is set to a truthy value
  if (process.env.PKO_VERBOSE) {
    setVerbose(true);
    debug(`Verbose logging enabled by $PKO_VERBOSE=${process.env.PKO_VERBOSE}`);
  }
  ...
  if (argv.length === 1) {
    switch (argv[0]) {
    case 'ls-overrides':
      debug(`Invoking pko command: ls-overrides`);
