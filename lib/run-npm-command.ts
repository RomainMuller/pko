...
export function runCommand(command: string, args: string[], additionalEnv?: NodeJS.ProcessEnv, silent: boolean = false): Promise<CommandResult> {
  return new Promise<CommandResult>((resolve, reject) => {
    try {
      ...
      // `shell: true` is required because on Windows, batch files must be run from a shell, and pko
      // is invoked using the batch file pko.cmd (to work around symlink issues on Windows).
