...
export async function determineStorageDirectory() {
  let result: string | undefined = process.env.PKO_REPOSITORY;
  if (!result) { result = await findOverlayDirectory(process.cwd()); }
  if (!result) { result = await findOverlayDirectory(installRoot); }
  if (!result) {
    throw new Error(`Unable to locate the ${overlayName} overlay directory, maybe you should set the PKO_REPOSITORY environment variable?`);
  }
  debug(`Storage directory: ${result} (override by setting the PKO_REPOSITORY environment variable)`);
